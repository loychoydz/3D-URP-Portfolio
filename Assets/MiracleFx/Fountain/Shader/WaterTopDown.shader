Shader "Unlit/WaterTopDown"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _NoisePow, _Disappear, _MaskOffset;
            ...
            v2f vert (appdata v)
            {
                ...
                float noise = tex2Dlod(_Noise, float4(o.uvNoise, 0, 0)).r;
                v.vertex.xyz += v.normal * noise * _NoisePow;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                i.uvMask.y += _MaskOffset * _Time.y;

                fixed4 col = tex2D(_MainTex, i.uv);
                fixed4 noise = tex2D(_Noise, i.uvNoise);
                fixed4 mask = tex2D(_Mask, i.uvMask);
                float3 edgeCol = (1,1,1);

                col *= _WaterCol;
                mask.a = smoothstep(_Disappear, _Disappear + 0.2, mask.a);
                col.rgb = lerp(edgeCol, col.rgb, mask.a); 
                col.a *= mask.a;
                return col;
            }
            ENDCG
